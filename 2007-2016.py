...
"""Create the 2007–2016 Metaverse Research Landscape video.
...
Expected repository layout (default):
    ...
    │   │   ├── metaverse_landscape_2008.jpeg
    │   │   └── ...
    │   └── videos/

The script discovers one image for every year from 2007 through 2016,
...
DEFAULT_IMAGE_SECONDS = 1.5
DEFAULT_ENDING_SECONDS = 3.0
OUTPUT_NAME = "Metaverse_Landscape_2007_2016.mp4"

TITLE = "Metaverse Research Landscape\n2007–2016"
OPENING_SUBTITLE = (
    "Virtual worlds, simulation, education, rehabilitation, and VR"
)
ENDING_TITLE = "2007–2016"
ENDING_SUBTITLE = (
    "The field was organized around immersive environments, simulation, "
    "education, rehabilitation, and VR before the later blockchain–AI expansion."
)
...
def make_card(size: int, title: str, subtitle: str) -> Image.Image:
    ...
    title_font = find_font(bold=True, size=max(24, round(size * 0.047)))
    body_font = find_font(bold=False, size=max(18, round(size * 0.031)))

    draw_centered_multiline(draw, round(size * 0.31), title, title_font, size, spacing=10)
    wrapped = "\n".join(textwrap.wrap(subtitle, width=48))
    draw_centered_multiline(draw, round(size * 0.51), wrapped, body_font, size, spacing=8)
    return image
...
def add_year_label(image: Image.Image, year: int, size: int) -> Image.Image:
    ...
    font = find_font(bold=True, size=max(24, round(size * 0.047)))
    label = str(year)
...
def build_video(args: argparse.Namespace) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="metaverse_2007_2016_") as temp_name:
